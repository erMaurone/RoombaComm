...
{
    private final Logger logger = Logger.getLogger(RoombaCommSerial.class);
     ...
     * useful, do:  
     *  com.hackingroomba.roombacomm.comm.flushOutput = true;
    ...
    public String[] listPorts() {
        ...
        try {
        	...
        	try {
                logger.info("make it or breake it ...");
        		portList = CommPortIdentifier.getPortIdentifiers();
                logger.info("got something = " + portList);
        	} catch (Exception e) {
                logger.info(e.getCause());
                errorMessage("listPorts1", e);
            }
            logger.info("got port list = " + portList);
            ...
            logger.info("unsatisfiedLinkError = " + e.getCause());
        } catch (Exception e) {
        ...
        // DEBUG
        logger.info("ports hashtable:");
        for( Enumeration e = portList; e.hasMoreElements(); ) {
            String p = (String) e.nextElement();
            Boolean b = (Boolean) ports.get(p);
            System.err.println("port:"+p+", inuse:"+b);
            logger.info("port = " + p + ", in use = " + b);
        }
   ...
   // -------------------------------------------------------------
     ...
     * FIXME: make it fail more gracefully, recognize bad port
     */
